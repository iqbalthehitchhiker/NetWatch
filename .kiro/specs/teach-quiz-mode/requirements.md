...
In Teach Mode, every interactive element exposes an explanatory panel describing what the metric or component represents and why it matters for diagnosis. There are no scores, no attempt limits, and no login required.

In Quiz Mode, the scored simulation + diagnosis-question flow is retained, but is now gated by real student authentication, a server-enforced per-lesson attempt counter, and an instructor-authenticated reset that restores attempt credit without erasing a previously recorded result.
...
- **Teach Mode**: Exploratory mode — no scoring, no attempt limits, no login. Explain_Panels visible.
- **Quiz Mode**: Scored, attempt-limited, identity-tied mode. Explain_Panels suppressed.
- **Mode_Selector**: The UI screen or toggle for choosing Teach or Quiz Mode before a lesson starts.
...
- **Diagnosis_Modal**: The end-of-lesson multiple-choice modal where the student selects a root-cause answer.
...
**User Story:** As a student or instructor, I want to choose between Teach Mode and Quiz Mode before starting a lesson simulation, so that the correct learning or assessment experience is applied.

#### Acceptance Criteria

1. WHEN a student selects a lesson card on the lesson-select screen, THE Mode_Selector SHALL present a choice between Teach Mode and Quiz Mode before the simulation loads.
2. THE Mode_Selector SHALL display a description of each mode of no more than 30 words per mode so the student can make an informed choice.
3. WHEN the student confirms a mode choice, THE Mode_Selector SHALL store the chosen mode in the current session state.
4. WHEN the student confirms a mode choice, THE Mode_Selector SHALL display a persistent mode indicator labelled "TEACH" or "QUIZ" in the simulation topbar for the duration of that session.
5. WHEN the back action is activated on the Mode_Selector, THE Mode_Selector SHALL return the student to the lesson-select screen without storing a mode choice in session state.
6. IF the student dismisses the Mode_Selector by pressing the Escape key or clicking outside the modal (without activating the back action and without confirming a mode), THEN THE Mode_Selector SHALL NOT load the simulation and SHALL return the student to the lesson-select screen without storing a mode choice.
7. WHILE Quiz Mode is active, THE Diagnosis_Modal SHALL NOT surface any hint after the first hint in the hint sequence.
8. WHILE Teach Mode is active, THE Diagnosis_Modal SHALL surface each additional hint when the student requests it, until all hints for the lesson have been displayed.
...
1. WHEN a student confirms Teach Mode, THE NetWatch SHALL launch the lesson without showing the Login_Screen and without making any backend authentication call.
...
1. WHEN Quiz Mode is selected on the Mode_Selector, THE Login_Screen SHALL be presented before the simulation loads.
...
7. WHEN the student activates the back action on the Login_Screen, THE NetWatch SHALL return to the Mode_Selector and SHALL clear any partially received token or cached credentials from memory.
